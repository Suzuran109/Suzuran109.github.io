...
(function () {
  ...
  function setTheme(config) {
    ...
    const icon =
      isAuto && defaultMode === "auto"
        ? "circle-half-stroke"
        : isDark
          ? "moon"
          : "sun";
    themeButton.id = `nav-${icon}-btn`;
  ...
  let savedMode = localStorage.getItem("dark_mode") || defaultMode;
  if (defaultMode !== "auto" && (savedMode === "auto" || savedMode === null)) {
  ...
  themeButton.addEventListener(
    ...
    throttle(() => {
      if (defaultMode === "auto") {
        ...
        setTheme(modes[(modes.indexOf(currentMode) + 1) % 3]);
      } else {
        const isDark =
